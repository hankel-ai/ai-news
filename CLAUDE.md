...
- **LLM outage gate + banner (2026-09-02)**: when AI analysis fails, the outage is persisted in the `settings` KV table as JSON row `llm_outage_state` (`{outage, since, reason, last_probe_at, skipped_runs}`) so the banner survives restarts. While active, each fetch run probes at most once/hour (`llm/probe.py`: Ollama `GET /api/tags`, others `GET /v1/models`) instead of firing full analysis batches at a dead/hung server (Ollama's 600s timeout per batch); the probe also verifies the configured **model is still installed** — a missing model counts as down. Probe ok → outage cleared AND the unanalyzed backlog drains (oldest-first, `BACKLOG_LIMIT=200` per run; the old code analyzed only stories from the current run, stranding outage-period stories forever). Frontend: `OutageBanner.tsx` on HeadlinesPage (main page only — `/embed` untouched), polls `GET /api/llm/status` every 60s; "Check now" calls `POST /api/llm/ping`, which clears the outage on success. `PUT /api/settings` returns 403 for `llm_outage_state` (`PROTECTED_KEYS`) — the settings API is public and must not be able to fake/clear the banner. Manual per-story ✨ and Settings backfill stay enabled during an outage (user-initiated, errors surface per-row). **Fixed while building this**: `POST /api/fetch` never passed `analysis_enabled`/LLM settings to `run_once` (defaults off) — manual triggers silently skipped analysis; scheduler's settings-gathering is now extracted to `scheduler._fetch_job_settings()` and shared by both paths. Gotcha: SQLAlchemy `session.delete()` doesn't emit DELETE until flush — a later `session.get()` in the same txn returns the stale identity-map row; flush immediately after delete. **Follow-up fix (2026-09-02, deployed as part of this feature):** the backlog drain exposed a latent crash — `analyzer._analyze_batch` passed **ORM** Story rows into `enrich_stories()`, which works on the `sources.base.Story` **dataclass** and reads `.published` (ORM column is `.published_at`) → `AttributeError` on any story with neither summary nor content, recorded by the new banner as an LLM outage. The old current-run-only analysis almost never hit that path (fresh stories carry summaries); draining old content-less rows hits it every run. Fix: `_analyze_batch` bridges ORM rows through dataclass proxies and copies `article_content` back; regression test `test_analyze_batch_enriches_orm_stories_without_content`.
